...
```{r}
readDirectorJson <- function(dir, nat) {
  read_json(
...
```{r}
movies <- bind_rows(
  readDirectorJson("Apichatpong Weerasethakul", "Thai"),
  readDirectorJson("Christopher Nolan", "UK/US"),
  readDirectorJson("Hirokazu Koreeda", "Japan"),
  readDirectorJson("Rajkumar Hirani", "India")
) |>
  tibble()
...
data <- inner_join(
  movies |>
    select(-year, -nationality, -opensubtitlePath, -image),
  subtitles |>
